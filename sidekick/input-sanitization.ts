...
const getCleanedContact = async (args: string[], client: Client, Starchat: Starchat) => {
    var jidNumber = '';
    ...
    if (parseInt(args[0]) === NaN || args[0][0] === "+" || args[0][0] === "@") {
        if (args[0][0] === "@" || args[0][0] === "+") {
            ...
            client.sendMessage(Starchat.chatId,"*Enter valid contact number.* Approved Syntax:\n```1. XXXXXXXXXX``` \n```2. Tag the person``` \n```3. +(YYY)XXXXXXXXXX.``` \n_(YY- Country Code, without zeros)_", MessageType.text);
            return undefined;
        }
    } else {
        jidNumber = args[0];       
...
const handleError = async (err, client, Starchat, customMessage = "```Something went wrong. The error has been logged in log chats```") => {
    console.log(chalk.redBright.bold("[ERROR] " + err));
    let data = {
        commandName: Starchat.commandName,
        fromMe: Starchat.fromMe,
        isReply: Starchat.isReply,
        isGroup: Starchat.isGroup,
        isPm: Starchat.isPm,
        isImage: Starchat.isImage,
        isBotGroupAdmin: Starchat.isBotGroupAdmin,
        isSenderGroupAdmin: Starchat.isSenderGroupAdmin,
        isSenderSudo: Starchat.isSenderSUDO,
        err: err
    }
    client.sendMessage(Starchat.chatId, customMessage, MessageType.text);
    client.sendMessage(Starchat.logGroup, format(ERROR_TEMPLATE, data), MessageType.text);
}
