...
async function main() {
    ...
    client.on('chat-update', async chat => {
        ...
        var Starchat = wa.resolve(chat, client, groupMetadata);
        // console.log(Starchat);
        if (Starchat.isCmd) {
            let isBlacklist = await Blacklist.getBlacklistUser(Starchat.sender, Starchat.chatId);
            const cleared = await clearance(Starchat, client, isBlacklist);
            if (!cleared) {
                return;
            }
            ...
            try {
                command.handle(client, chat, Starchat, args).catch(err => console.log("[ERROR] " + err));
            } catch (err) {
                console.log(chalk.red("[ERROR] ", err));
            }
        }
