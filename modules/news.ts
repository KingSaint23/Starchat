...
module.exports = {
    ...
    async handle(
        ...
        if (args.length === 0) {
            await client
                .sendMessage(Starchat.chatId, NEWS.NO_COMMMAND, MessageType.text)
        ...
        if (args[0] == "search") {
            ...
            await client
                .sendMessage(Starchat.chatId, message, MessageType.text)
                .catch((err) => inputSanitization.handleError(err, client, Starchat));
            return;
        }
        if (args[0] == "fetch") {
            ...
            if (!searchTerm) {
                await client
                    .sendMessage(Starchat.chatId, NEWS.NO_PUB_NAME, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, Starchat));
                return;
            }
            let searchResponse;
            ...
            let message =
                ...
                foundPub +
                "* ```is being fetched by Starchat, this may take some time, please be patient!```";
            if (!foundPub) {
                message = "```Sorry, no publication found by that name!```";
                await client
                    .sendMessage(Starchat.chatId, message, MessageType.text)
            ...
            await client
                ...
                .catch((err) => inputSanitization.handleError(err, client, Starchat));
            message = "```Your requested publication fetched by Starchat``` ☝️";
            await client
                .sendMessage(Starchat.chatId, message, MessageType.text)
                .catch((err) => inputSanitization.handleError(err, client, Starchat));
            return;
