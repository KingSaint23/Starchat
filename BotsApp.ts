            ...
            async (events) => {
                if (events['connection.update']) {
                    ...
                    if (connection === 'close') {
                        if ((lastDisconnect.error as Boom)?.output?.statusCode !== DisconnectReason.loggedOut) {
                            ...
                            console.log(chalk.redBright('Connection closed. You are logged out. Delete the Starchat.db and session.data.json files to rescan the code.'));
                            process.exit(0);
                        }
                    } else if (connection === 'connecting') {
                        console.log(chalk.yellowBright("[INFO] Connecting to WhatsApp..."));
                    } else if (connection === 'open') {
                        console.log(chalk.greenBright.bold("[INFO] Connected! Welcome to Starchat"));
                    }
                }
                ...
                if (events['messages.upsert']) {
                    ...
                    for(const msg of upsert.messages){
                        ...
                        let Starchat: Starchat = await resolve(chat, sock);
                        // console.log(Starchat);
                        if (Starchat.isCmd) {
                            let isBlacklist: boolean = await Blacklist.getBlacklistUser(Starchat.sender, Starchat.chatId);
                            const cleared: boolean = await clearance(Starchat, client, isBlacklist);
                            if (!cleared) {
                                return;
                            }
                            ...
                            console.log(chalk.redBright.bold(`[INFO] ${Starchat.commandName} command executed.`));
                            const command = commandHandler.get(Starchat.commandName);
                            var args = Starchat.body.trim().split(/\s+/).slice(1);
                            if (!command) {
                                client.sendMessage(Starchat.chatId, "```Woops, invalid command! Use```  *.help*  ```to display the command list.```", MessageType.text);
                                return;
                            } else if (command && Starchat.commandName == "help") {
                                try {
                                    command.handle(client, chat, Starchat, args, commandHandler);
                                    return;
                                } catch (err) {
                            ...
                            try {
                                await command.handle(client, chat, Starchat, args).catch(err => console.log("[ERROR] " + err));
                            } catch (err) {
                                console.log(chalk.red("[ERROR] ", err));
                            }
                        }
